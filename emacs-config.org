...
If you cannot access github because of GFW, please add git proxy as following:
...
(use-package org
  ...
  (org-level-2 ((t (:height 1.24 :foreground "#C57BDB" :weight bold))))
  (org-level-3 ((t (:height 1.18 :foreground "#a991f1" :weight bold))))
  (org-level-4 ((t (:height 1.12 :foreground "#7cc3f3" :weight bold))))
  (org-level-5 ((t (:height 1.09 :foreground "#d39ce3" :weight bold))))
  (org-level-6 ((t (:height 1.06 :foreground "#a8d7f7" :weight bold))))
  (org-level-7 ((t (:height 1.03 :foreground "#e2bded" :weight bold))))
  (org-level-8 ((t (:height 1.0 :foreground "#dceffb" :weight bold))))
...
#+end_src
** Org appear
Toggle visibility of *emphasis markers*, links, /subscripts/, and superscripts by customising variables such as org-hide-emphasis-markers. However, it is currently not possible to do this interactively and on an element-by-element basis. [[https://github.com/awth13/org-appear][org-appear]] will solve this.
#+begin_src emacs-lisp
...
#+end_src
** Online images and youtube videos
Please see [[https://github.com/TobiasZawada/org-yt][org-yt]] for details.
#+begin_src emacs-lisp -n
(require 'org-yt)
...
(use-package org-tempo
  ...
  (setq org-structure-template-alist
        '(("s" . "src")
          ...
          ("json" . "src json")
          ("jp"  . "src jupyter-python :session py :file temp.py")
          ("http" . "src http :pretty")
          ("ein" . "src ein-python :session localhost :results output :file temp.py")
          ("py" . "src python :results output :file temp.py")))
  ...
  (tempo-define-template "org-options-toc"
                         '( "#+OPTIONS: toc:nil num:4 H:4 ^:nil pri:t" n p)
                         "<toc"
                         "Insert options toc settings" 'org-tempo-tags)
  (tempo-define-template "org-html-head"
                         '( "#+HTML_HEAD: <link rel=\"stylesheet\" type=\"text/css\" href=\"http://gongzhitaao.org/orgcss/org.css\"" n p)
                         "<hh"
                         "Insert html head settings" 'org-tempo-tags)
...
(defun org-export-output-file-name-modified (orig-fun extension &optional subtreep pub-dir)
  (unless pub-dir
    ...
    ;; (setq pub-dir (concat (buffer-file-name) ".assets/"))
...
#+end_src
**** TWBS
Twitter Bootstrap style HTML backend.
#+begin_src emacs-lisp
...
#+end_src
**** GFM
Github Flavored Markdown backend.
#+begin_src emacs-lisp
...
1. install =graphviz= package first.
   #+begin_src shell :results output :file: temp.sh
...
(use-package org-mind-map
  ...
  (setq org-mind-map-include-text t)
   ...
   #+end_src
**** org-opml
Once org-opml is installed and you open an OPML file, it converts the OPML to Org mode style text.
Please see [[GitHub:org-opml/org-opml][org-opml]] for details.

#+begin_src emacs-lisp
(use-package org-opml
  :quelpa ((org-opml :fetcher github
                     :repo "org-opml/org-opml")
           ;; :upgrade t
           )
  ;; :load-path "~/.emacs.d/quelpa/build/org-opml"
  )
#+end_src
...
#+end_src
*** Automatically upload image when exporting to markdown files
#+begin_src emacs-lisp -n
(defun upload-image-files-when-exporting-to-gfm-markdown (org &rest args)
  "When exporting org files to gfm like markdown files, upload all images files and
convert all links to online links"
  (interactive)
  (setq mdPath (concat (file-name-directory (buffer-file-name)) "exported/" (file-name-sans-extension (file-name-nondirectory (buffer-file-name))) ".md"))
  (shell-command (concat "python /Users/randolph/iCloud/bin/python_script/auto_upload_img.py " mdPath " -noupload"))
  )

(advice-add 'org-gfm-export-to-markdown :after 'upload-image-files-when-exporting-to-gfm-markdown)
(advice-add 'org-md-export-to-markdown :after 'upload-image-files-when-exporting-to-gfm-markdown)
(advice-add 'org-pandoc-export-to-markdown :after 'upload-image-files-when-exporting-to-gfm-markdown)
#+end_src
** Blogging via Hugo
*** hugo preparation
**** install hugo first
...
(defun my-org-insert-clipboard-image ()
 ...
 (%filename.assets) as the org-buffer and insert a link to this file."
  ...
  ;; should not use relativePath, because when export, image will not be shown.
...
|-----------+---------------------------------------------------------------------|
| C-c C-l   | display detailed link when point is over an existing link           |
...
Lsp mode has an official function =lsp-org= to support this.
#+begin_src emacs-lisp
